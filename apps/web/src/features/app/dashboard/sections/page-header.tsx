...
import type { OverviewPeriod } from "@/features/app/analises/data";
    ...
    <div className="flex flex-col gap-6 border-b border-gray-100 pb-8 md:flex-row md:items-end md:justify-between">
      <div className="min-w-0 flex-1">
        ...
        <h1 className="text-4xl font-medium leading-[1.02] tracking-tighter text-ink md:text-[3.5rem]">
          {greeting}, {firstName} —{" "}
          <span className="font-serif text-[1.1em] font-normal italic text-brand-primary">
            vamos ensinar
          </span>
        </h1>
        <p className="mt-3.5 max-w-md text-[15px] leading-relaxed text-gray-500">
          Visão geral das suas turmas no período selecionado.
        </p>
      </div>

      <PeriodFilter active={period} baseHref="/app" />
    </div>
  );
}
