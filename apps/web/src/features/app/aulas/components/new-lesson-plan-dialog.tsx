...
import { NotebookPen, Plus, Search, Users } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
interface NewLessonPlanDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

// Espelha o NewExamDialog: escolhe a turma e leva pro wizard de plano de aula.
export function NewLessonPlanDialog({
  open,
  onOpenChange,
}: NewLessonPlanDialogProps) {
  ...
  function pickTurma(turmaId: string) {
    onOpenChange(false);
    router.push(`/app/turmas/${turmaId}/aulas/nova`);
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Qual turma vai receber a aula?</DialogTitle>
          <DialogDescription>
            Escolha a turma. Nas etapas seguintes você escolhe o segmento e
            monta o plano.
          </DialogDescription>
        </DialogHeader>
    ...
    <div className="flex flex-col items-center gap-4 py-6 text-center">
      ...
      <div className="flex flex-col gap-1">
        ...
        <p className="max-w-xs text-xs text-gray-500">
          Antes de criar um plano de aula, precisa ter uma turma onde ele vai
          ficar guardado.
        </p>
      </div>
